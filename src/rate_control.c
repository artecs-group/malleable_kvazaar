...
void kvz_set_picture_lambda_and_qp(encoder_state_t * const state)
{
	...
	if (state->frame->num > ctrl->cfg.owf) {
	    ...
	    /*@ @*/
	    puts(" ESTE CASO NO SE HA CONTEMPLADO. ERROR: " \
		 __FILE__				    \
	...
	if (gop_len > 0 && state->frame->slicetype != KVZ_SLICE_I) {
	    // clip to qp transforma el valor entre 0 y 51
	...
	//int localqp = (state->frame->num % 100)/2;

	int localqp = SS_ask_QP(state->frame->num+1);
	//int localqp = -1;
	
	if(localqp > 0){
	    state->frame->QP = localqp;
//	  if (gop_len > 0 && state->frame->slicetype != KVZ_SLICE_I) {
//	    state->frame->QP = CLIP_TO_QP(localqp + gop->qp_offset);
//	  } else {
//	    state->frame->QP = localqp;
//	  }
	} else {
                               ...
                               vector2d_t pos)
{
	...
	/*@ NOTA: parece ser que estos valores se modifican de acuerdo a los valores dados para
	       QP en la funcion anterior, por lo que no es necesario modificar nada aqui
	  @*/
