...
static void* threadqueue_worker(void* threadqueue_opaque)
{
    ...
    /* /\*@ Notify the server about the new Thread @*\/ */
    /* SS_addNewThread(syscall(SYS_gettid)); */
    /* /\*@ @*\/ */
  ...
  for (;;) {
    while ( (!threadqueue->stop && threadqueue->first == NULL) || 
            (!threadqueue->stop && (threadqueue->thread_active > threadqueue->thread_max_active) ) ) {
    ...
    /* KVZ_CLOCK_T start_timestamp, end_timestamp; */
    /* KVZ_GET_TIME(&start_timestamp); */
    /* //Procesado del frame */

    
    job->fptr(job->arg);

    /* KVZ_GET_TIME(&end_timestamp); */

    /* printf("time_%u = [ time_%u %Ld ];\n", pthread_self(), pthread_self(), */
    /* 	   KVZ_CLOCK_T_NS_AS_LONG_LONG(end_timestamp) - KVZ_CLOCK_T_NS_AS_LONG_LONG(start_timestamp)); */
...
threadqueue_queue_t * kvz_threadqueue_init(int thread_count)
{
  ...
  //threadqueue->thread_max_active = 55;
#if defined(MULHACEN) || defined(mulhacen)
  threadqueue->thread_max_active = 68;
#else
  threadqueue->thread_max_active = sysconf(_SC_NPROCESSORS_ONLN);
#endif
...
void kvz_threadqueue_set_active_threads(threadqueue_queue_t * threadqueue, int num_threads)
{
    PTHREAD_LOCK(&threadqueue->lock);
    threadqueue->thread_max_active = num_threads;
      //fprintf( stderr, "Adding one active thread (%d/%d)\n", threadqueue->thread_active, threadqueue->thread_max_active );
    PTHREAD_UNLOCK(&threadqueue->lock);
}


void kvz_threadqueue_add_active_threads(threadqueue_queue_t * threadqueue, int num_threads)
{
  PTHREAD_LOCK(&threadqueue->lock);
      if( threadqueue->thread_max_active < threadqueue->thread_count ) {
        threadqueue->thread_max_active += num_threads;
        //fprintf( stderr, "Adding one active thread (%d/%d)\n", threadqueue->thread_active, threadqueue->thread_max_active );
      }
  PTHREAD_UNLOCK(&threadqueue->lock);
...
void kvz_threadqueue_remove_active_threads(threadqueue_queue_t * threadqueue, int num_threads)
{
  PTHREAD_LOCK(&threadqueue->lock);
      if( threadqueue->thread_max_active > 1 ) {
        threadqueue->thread_max_active -= num_threads;
        //fprintf( stderr, "Removing one active thread (%d/%d)\n", threadqueue->thread_active, threadqueue->thread_max_active );
      }
  PTHREAD_UNLOCK(&threadqueue->lock);
}
