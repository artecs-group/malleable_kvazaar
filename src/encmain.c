...
int main(int argc, char *argv[])
{
    ...
    //KVZ_CLOCK_T encoding_sofar_real_time;

    /* clock_t frame_encoding_end_cpu_time; */
    /* clock_t frame_encoding_start_cpu_time; */

    KVZ_CLOCK_T frame_encoding_start_real_time;
    KVZ_CLOCK_T frame_encoding_end_real_time;
	...
	for (;;) {
		...
		PTHREAD_UNLOCK(&encoder->threadqueue->lock);
	    }
	    /*@ @*/
	    ...
	    /* frame_encoding_end_cpu_time = clock(); */
	    KVZ_GET_TIME(&frame_encoding_end_real_time);
	    stats_end_frame(encoder->stats, frame_encoding_end_real_time);
		...
		psnr_sum[2] += frame_psnr[2];
                ...
                /*@ --- @*/
		//SS_updateFrameInfo(frames_done, 2, len_out, frame_psnr, KVZ_CLOCK_T_NS_AS_LONG_LONG(frame_encoding_end_real_time));
		/*@   @*/		
		
	    } // end frame? ( chunks_out != NULL )
