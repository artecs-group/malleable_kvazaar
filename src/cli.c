...
static const struct option long_options[] = {
    ...
    { "target_fps",         required_argument, NULL, 0 },
...
void print_frame_info(const kvz_frame_info *const info,
                      ...
                      const long double bitrate,
                      const bool print_psnr,
                      stats_t *st)
{
#ifdef VERBOSE
    fprintf(stderr, "POC %4d QP %2d (%c-frame) %Lf bits",
	    info->poc,
	    info->qp,
    ...
    // OPCION A
     ...
     //    [poc_id, qp, slice_type (BPI), bits, psnr_y, psnr_u, psnr_v, fps_sofar)
    /* fprintf(stderr, " info( %4d, 1:8 ) = [ %4d, %2d, %d, %10d, %2.4f, %2.4f, %2.4f, %f ];\n", */
    /* 	    st->frames_sofar, info->poc, info->qp, info->slice_type % 3, bytes << 3, */
    /* 	    frame_psnr[0], frame_psnr[1], frame_psnr[2], st->fps_sofar);	     */


    // OPCION B
    /* KVZ_CLOCK_T current_timestamp; */
    /* KVZ_GET_TIME(&current_timestamp); */
    
    /* fprintf(stderr, " info = [ info; [%lf, %4d, %2d, %d, %Lf, %2.4f, %2.4f, %2.4f, %f ]];\n", */
    /* 	    KVZ_CLOCK_T_AS_DOUBLE(current_timestamp), */
    /* 	    st->frames_sofar, info->qp, info->slice_type % 3, bitrate, */
    /* 	    frame_psnr[0], frame_psnr[1], frame_psnr[2], st->fps_sofar */
    /* 	); */
