...
encoder_control_t* kvz_encoder_control_init(const kvz_config *const cfg)
{
  ...
  /*@ @*/
  /* encoder->stats = kvz_stats_init( 0, 0, encoder->threadqueue, */
  /* 				   encoder->cfg.target_fps, */
  /* 				   encoder->cfg.qp, */
  /* 				   encoder->in.real_height, encoder->in.real_width, */
  /* 				   encoder->cfg.owf, */
  /* 				   encoder->cfg.preset); */
  /*@ @*/
  encoder->stats = kvz_stats_init(encoder->threadqueue);
  
  if (!encoder->stats) {
      fprintf(stderr, "Could not initialize stats module.\n");
      goto init_failed;
  }
